...
def evaluate_match(profile: CompanyProfile, analysis: TenderAnalysisOutput) -> MatchReport:
    ...
    passed_m = sum(1 for r in mandatory if r.passed)
    failed_m = sum(1 for r in mandatory if not r.passed)
    passed_o = sum(1 for r in optional if r.passed)
    failed_o = sum(1 for r in optional if not r.passed)

    is_eligible = failed_m == 0

    # Score: 60 % weight on mandatory pass-rate + 40 % on overall pass-rate
    total = len(breakdown)
    if total == 0:
        score = 0.0
    else:
        m_rate = passed_m / len(mandatory) if mandatory else 1.0
        all_rate = (passed_m + passed_o) / total
        score = round((0.6 * m_rate + 0.4 * all_rate) * 100, 1)

    if is_eligible:
        summary_he = (
            f"החברה עומדת בכל {passed_m} תנאי הסף המחייבים. "
            f"ציון התאמה: {score}/100."
        )
    else:
        failed_items = [r for r in mandatory if not r.passed]
        ...
        summary_he = (
            f"החברה אינה עומדת ב-{failed_m} תנאי סף מחייבים: {short_descs}{suffix}. "
            f"ציון התאמה: {score}/100."
        )
